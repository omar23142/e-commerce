...
import { ApiConsumes, ApiBody, ApiSecurity } from "@nestjs/swagger";
import { imagesUploadDto } from "./dto/Image-UploadDto.dto";
import { CloudinaryService } from "./cloudinary.service"; // تأكد من المسار الصحيح
...
export class UploadsController {
    ...
    @ApiSecurity('bearer')
    @UseInterceptors(FilesInterceptor('files'))
    public async uploadFiles(@UploadedFiles() files: Array<Express.Multer.File>, @GetCurrentUser() user: user) {
        if (!files || files.length === 0) {
